...
bot.on('message', function (user, userID, channelID, message, evt) {
  if (message.startsWith(prefix)) {
    ...
    switch (command) {
      case 'roll':
        sendDelayedMessage(channelID, rollDie(args), 500);
        break;
    }
  }
});
...
/**
 * Generates a rolled die message based on provided arguments.
 * @param {String[]} args An array of strings containing the parameters for rolling the die. [[rollCount]d[faceCount], modifer, modifierVal].
 */
    ...
    if ((beforeD != '' && !(/^\d+$/).test(beforeD)) || afterD == '' || !(/^\d+$/).test(afterD)) {
      ...
      if (!dieOptions.includes(faceCount)) {
        ...
        for (i = 0; i < rollCount; i++) {
          ...
          // For rollCount 1, don't calculate a total or output a "total". It's just one value.
          if (rollCount == 1) {
            if (faceCount == 20 && (result == 1 || result == 20)) {
              // Special message for critical fails and natural 20s on d20.
              if (result == 1) {
